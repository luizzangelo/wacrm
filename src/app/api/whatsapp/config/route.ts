...
async function resolveAccountId(
  ...
  if (
    error ||
    !data?.account_id ||
    !isAccountRole(data.account_role) ||
    !hasMinRole(data.account_role, minRole)
  )
    return null;
...
export async function GET() {
  try {
    ...
    const { data: config, error: configError } = await supabase
      ...
      .select(
        'phone_number_id, access_token, status, connection_mode, subscription_status, app_state_sync_status, history_sync_status, is_on_biz_app, platform_type, disconnected_at'
      )
    ...
    try {
      ...
      return NextResponse.json({
        ...
        connection_mode: config.connection_mode ?? 'manual',
        subscription_status: config.subscription_status ?? 'unknown',
        sync: {
          app_state: config.app_state_sync_status ?? 'not_requested',
          history: config.history_sync_status ?? 'not_requested',
        },
        is_on_biz_app: config.is_on_biz_app ?? null,
        platform_type: config.platform_type ?? null,
      ...
      console.error(
        ...
        operationalErrorFields(message)
      );
      return NextResponse.json(
        {
          connected: false,
  ...
  try {
    ...
    if (claimedError) {
      console.error(
        ...
        operationalErrorFields(claimedError)
      );
      return NextResponse.json(
      ...
      if (!pin) {
        ...
        try {
          ...
          console.error(
            ...
            operationalErrorFields(registrationError)
          );
    ...
    const baseRow = {
      ...
      subscription_status: subscribedAppsAt ? 'subscribed' : 'unknown',
    ...
    if (existing) {
      ...
      if (updateError) {
        console.error(
          ...
          operationalErrorFields(updateError)
        );
        return NextResponse.json(
      ...
      if (insertError) {
        console.error(
          ...
          operationalErrorFields(insertError)
        );
        return NextResponse.json(
...
export async function DELETE() {
  try {
    ...
    if (existing?.connection_mode === 'coexistence') {
      return NextResponse.json(
        {
          error: 'coexistence_disconnect_required',
          message:
            'Use the coexistence disconnect flow so CRM history and connection state are preserved.',
        },
        { status: 409 }
      );
    }
    ...
    if (deleteError) {
      console.error(
        ...
        operationalErrorFields(deleteError)
      );
      return NextResponse.json(
