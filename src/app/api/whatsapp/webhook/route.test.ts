          ...
          return {
            select: () => ({
              eq: (_column: string, phone: string) =>
                Promise.resolve({
                  data:
                    h.state.phoneAccounts && !h.state.phoneAccounts[phone]
                      ? []
                      : [
                          {
                            id: 'whatsapp-config-1',
                            account_id:
                              h.state.phoneAccounts?.[phone] ?? 'acc-1',
                            user_id: 'user-1',
                            access_token: 'enc',
                            waba_id: 'waba-1',
              ...
              eq: (_column: string, messageId: string) => {
                return {
                  eq: (_column: string, accountId: string) => {
                    ...
                    for (const stored of h.state.storedStatuses)
                      if (
...
describe('WhatsApp webhook: global callback verification token', () => {
  it('accepts the server-side app token without reading tenant configs', async () => {
    vi.stubEnv('WHATSAPP_WEBHOOK_VERIFY_TOKEN', 'global-test-token');
    const response = await GET(
      new Request(
        'https://crm.example/api/whatsapp/webhook?hub.mode=subscribe&hub.challenge=challenge-123&hub.verify_token=global-test-token'
      )
    );
...
describe('WhatsApp webhook: unmatched delivery-status observability', () => {
  ...
  it('keeps correlated message status behavior and emits no unmatched log', async () => {
    ...
    expect(h.state.messageStatusUpdates).toEqual([
      {
        status: 'delivered',
        messageId: 'wamid.CORRELATED-12345678',
        accountId: 'acc-1',
      },
    ]);
...
describe('WhatsApp multi-tenant routing (21G)', () => {
  ...
  it('status for Phone A changes only A, even when B has the same external ID', async () => {
    ...
    expect(h.state.storedStatuses.map((row) => row.status)).toEqual([
      'delivered',
      'sent',
    ]);
    expect(h.state.messageStatusUpdates).toEqual([
