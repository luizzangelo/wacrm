...
function verifyTokenMatches(expected: string, supplied: string): boolean {
...
export async function GET(request: Request) {
  try {
    ...
    // A SaaS Meta App has one callback-level verification token. Embedded
    // Signup customers must never type it. Keep the encrypted per-account
    // scan below as a backwards-compatible fallback for manual configs.
    const globalVerifyToken = process.env.WHATSAPP_WEBHOOK_VERIFY_TOKEN?.trim();
    if (
      globalVerifyToken &&
      verifyTokenMatches(globalVerifyToken, verifyToken)
    ) {
    ...
    // Fetch all whatsapp configs to check legacy per-account verify tokens
  ...
  for (const entry of body.entry) {
    for (const change of entry.changes) {
      ...
      try {
        ...
        console.error('WhatsApp config decryption failed for matched tenant');
        continue;
      }
...
async function handleStatusUpdate(
  ...
  if (recFetchErr) {
    console.error(
      ...
      operationalErrorFields(recFetchErr)
    );
  } else if (
    recipient &&
    ...
    if (recUpdateErr) {
      console.error(
        ...
        operationalErrorFields(recUpdateErr)
      );
    }
  }
...
async function handleReaction(
  ...
  if (!reaction.emoji) {
    ...
    if (delError) {
      console.error(
        ...
        operationalErrorFields(delError)
      );
    }
    return;
  }
  ...
  if (upsertError) {
    console.error(
      ...
      operationalErrorFields(upsertError)
    );
  }
}
...
async function processMessage(
  ...
  if (convError) {
    console.error(
      ...
      operationalErrorFields(convError)
    );
  }
...
async function findOrCreateContact(
  ...
  if (createError) {
    ...
    console.error(
      ...
      operationalErrorFields(createError)
    );
    return null;
  }
...
async function findOrCreateConversation(
  ...
  if (findError) {
    console.error(
      ...
      operationalErrorFields(findError)
    );
    return null;
  }
