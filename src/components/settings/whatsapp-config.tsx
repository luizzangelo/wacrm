...
export function WhatsAppConfig() {
  ...
  }>({ displayNumber: null, verifiedName: null });
      ...
      try {
        ...
        if (data) {
          try {
            ...
            setVerifiedPhone({
              displayNumber: payload.phone_info?.display_phone_number ?? null,
              verifiedName: payload.phone_info?.verified_name ?? null,
            });
          ...
          setVerifiedPhone({ displayNumber: null, verifiedName: null });
          setConnectionStatus('disconnected');
          setResetReason(null);
          setStatusMessage('');
        }
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
        ...
        <div className="space-y-6">
          <WhatsAppEmbeddedSignup
            ...
            displayNumber={verifiedPhone.displayNumber}
            verifiedName={verifiedPhone.verifiedName}
            subscriptionStatus={config?.subscription_status}
            appStateSyncStatus={config?.app_state_sync_status}
            historySyncStatus={config?.history_sync_status}
            onChanged={async () => {
              if (accountId) await fetchConfig(accountId);
            }}
          />
          ...
          {config && config.connection_mode !== 'coexistence' && (
            <Alert
              className={
                isRegistered
          ...
          {/* Advanced manual fallback. Coexistence customers never see or edit
            access/verify tokens; their lifecycle is managed by the card above. */}
          {canEditSettings && config?.connection_mode !== 'coexistence' && (
            <Card>
              <CardHeader>
                ...
                <CardDescription className="text-muted-foreground">
                  Fallback para integrações Cloud API existentes. O fluxo
                  recomendado é Conectar WhatsApp acima.
                </CardDescription>
              </CardHeader>
                  ...
                  <AccordionItem className="border-border">
                    ...
                    <AccordionContent className="text-muted-foreground">
                      <ol className="list-inside list-decimal space-y-1 text-sm">
                        <li
                          dangerouslySetInnerHTML={{ __html: t('step1_1') }}
                        />
                        <li>{t('step1_2')}</li>
                        <li>{t('step1_3')}</li>
                        <li>{t('step1_4')}</li>
