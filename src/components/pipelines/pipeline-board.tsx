...
export function PipelineBoard({
  ...
  const sortedStages = useMemo(() => sortDealStages(stages), [stages]);
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (!target) return;
    if (needsLossConfirmation(target)) {
      setPendingLoss({ dealId, stageId: targetStageId });
      return;
    }
      ...
      {pendingLoss && (
        <LostReasonDialog
          ...
          onConfirm={async (loss) => {
            await onDealMoved(pendingLoss.dealId, pendingLoss.stageId, loss);
            setPendingLoss(null);
          }}
        />
      )}
    ...
    <div className="border-border bg-card/60 flex w-[85vw] max-w-[320px] min-w-[260px] shrink-0 snap-start flex-col rounded-xl border p-4 lg:w-auto lg:max-w-none lg:flex-1 lg:shrink lg:basis-[260px] lg:snap-none">
      ...
      <div className="flex items-center justify-between pt-3">
        <h3 className="text-foreground truncate text-sm font-semibold">
          {stage.is_lost_stage ? lossT('stage') : stage.name}
        </h3>
